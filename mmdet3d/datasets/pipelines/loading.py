...
class LoadFlowGT(object):
    def __init__(self, data_root, render_frame_ids=[-1, 0, 1], use_classes = [2,3,4,5,6,7,9,10]):
        self.data_root = data_root
        self.render_frame_ids = render_frame_ids
        gt_render_frame_ids = [-3, -2, -1, 1, 2, 3] # horizon of the precomputed gt = [-3,...,3]

        self.load_indices = torch.tensor([i for i,t in enumerate(gt_render_frame_ids) if t in self.render_frame_ids]).unsqueeze(0)
        self.temporal_index_map = torch.tensor([self.render_frame_ids.index(i) if i in self.render_frame_ids else 0 for i in gt_render_frame_ids])

        self.use_classes = torch.tensor(use_classes).unsqueeze(0)

    def __call__(self, results):
        
        # Load flow GT from file
        target_file = os.path.join(self.data_root, results['scene_name'], f'{results["sample_idx"]}.npz') 
        flow_file = np.load(target_file)
        flow_gt = torch.tensor(flow_file['flow'])
        transforms_t = torch.tensor(flow_file['T'])
        box_indices = torch.tensor(flow_file['indices'].astype(np.int16))

        # Take only needed timesteps
        temporal_mask = (flow_gt[:, -1].unsqueeze(-1) == self.load_indices).any(-1)

        # Adapt the temporal indices to fit current setting
        flow_gt[:, -1] = self.temporal_index_map[flow_gt[:, -1].long()] 

        # select timesteps for current setting
        flow = flow_gt[temporal_mask]

        # filter by classes we want to move
        class_mask = (flow[:, -2].unsqueeze(1) == self.use_classes).any(-1)

        results['voxel_flow'] = flow[class_mask]
        results['flow_transforms'] = transforms_t
        results['box_indices'] = box_indices[temporal_mask][class_mask]

        return results

@PIPELINES.register_module()
class GenerateRays(object):
    def __init__(self,  depth_range, file_client_args=dict(backend='disk'), render_frame_ids=[0], pc_range=[-40., -40., -1.0, 40., 40., 5.4], semantics=True, ignore_classes=None):
        ...
        self.load_dim=5
        self.use_dim=list(range(5))
        self.render_frame_ids = render_frame_ids
        self.zero_index = [i for i, t in enumerate(render_frame_ids) if t==0][0]
        self.lidarseg_map = np.array([0, 0, 7, 7, 7, 7, 7, 0, 0, 1, 0, 0, 8, 0, 2, 3, 3, 4, 5, 0, 0, 6, 9, 10, 11, 12, 13, 14, 15, 0, 16, 0], dtype=np.uint8)
        self.ignore_classes = torch.tensor(ignore_classes) if ignore_classes is not None else None

    def _load_points(self, pts_filename):
        if self.file_client is None:
            self.file_client = mmcv.FileClient(**self.file_client_args)
        
        if pts_filename.endswith('.npz'): # panoptic labels
            points = np.load(pts_filename)['data']
    ...
    def calculate_transformations(self, results):
        ...
        # need: Cam2ego_l_t = cam2global * global2ego_l_t
        ego_l_t2global = lidarego2global[self.zero_index]
        ...
        return lidar2img, cam2ego_lidar_t, lidar2ego_l_t#, intrins_post

    def __call__(self, results):
        
        # Load points
        points = [self._load_points(frame['lidar_path']).reshape(-1, self.load_dim)[:, self.use_dim] for frame in results['render_frames']]
        sem_points = [self._load_points(frame['panoptic_path']) for frame in results['render_frames']]
        lidarseg_points = [self.lidarseg_map[(sem_point // 1000).astype(np.uint8)] for sem_point in sem_points]
        _, _, H, W = results['img_inputs'][0].shape
        intrins, post_rots, post_trans, bda = results['img_inputs'][3:]
        fx = intrins[:6, 0, 0]
        fy = intrins[:6, 1, 1]
        cx = intrins[:6, 0, 2]
        cy = intrins[:6, 1, 2]
        ...
        # create relevant transformations
        lidar2img, cam2ego_lidar_t, lidar2ego_l_t = self.calculate_transformations(results)

        for i, points_t in enumerate(points):
            t = self.render_frame_ids[i]

            # Filter out points that are outside of the current occupancy grid (requires transforming points to current ego_l)
            points_in_ego_l_t = (lidar2ego_l_t[i, :3, :3] @ points_t[:, :3, None]).squeeze(-1) + lidar2ego_l_t[i, :3, 3]
            oob_mask = (points_in_ego_l_t[:, 0] > self.pc_range[0]) & (points_in_ego_l_t[:, 0] < self.pc_range[3]) & (
                            points_in_ego_l_t[:, 1] > self.pc_range[1]) & (points_in_ego_l_t[:, 1] < self.pc_range[4]) & (
                            points_in_ego_l_t[:, 2] > self.pc_range[2]) & (points_in_ego_l_t[:, 2] < self.pc_range[5])
            points_t = points_t[oob_mask]

            # Project points on images
            points_img = (lidar2img[i, ..., None, :3, :3] @ points_t[None, :, :3, None]).squeeze(-1) + lidar2img[i, ..., None, :3, 3]
            points_img[..., :2] /= points_img[..., 2:3]
            coor_pre = points_img.clone()[..., :2]
            # Apply post rot & trans
            points_img = torch.matmul(post_rots[:6, None, ...], points_img.unsqueeze(-1)).squeeze(-1) + post_trans[:6, None]

            coor = points_img[..., :2]
            depth = points_img[..., 2]
            kept = (coor[..., 0] >= 0) & (coor[..., 0] < W) & (
                coor[..., 1] >= 0) & (coor[..., 1] < H) & (
                    depth < self.depth_range[1]) & (
                        depth >= self.depth_range[0])
            coor, depth, coor_pre = coor[kept], depth[kept], coor_pre[kept]

            semantics = torch.tensor(lidarseg_points[i][oob_mask][None, ...].repeat(kept.shape[0], 0)[kept]) if self.semantics else None
            cam_idx = kept.nonzero()[:, :1]
            cam_idx = torch.cat((torch.full(cam_idx.shape, i), cam_idx), axis=1)

            # Dynamic object filtering (if not using flow)
            if t != 0 and self.ignore_classes is not None and self.semantics:
                invalid_mask = ~((semantics[..., None] == self.ignore_classes).any(-1))
                cam_idx = cam_idx[invalid_mask]
                semantics = semantics[invalid_mask]
                coor = coor[invalid_mask]
                depth = depth[invalid_mask]
                coor_pre = coor_pre[invalid_mask]
                
            # Generate rays (origins + directions)
            x, y = coor_pre[:, 0], coor_pre[:, 1]

            mask_indices = tuple(cam_idx.T)[1]
            cam_space_x = (x+0.5 - cx[mask_indices]) / fx[mask_indices]
            cam_space_y = (y+0.5 - cy[mask_indices]) / fy[mask_indices]
            ...
            directions = (cam2ego_lidar_t[i][mask_indices][:, :3, :3] @ ray_dirs_cam[..., None]).squeeze(-1)
            ...
            all_origins.append(origins) # [#cams, 3]
            all_directions.append(directions) # [nrays, 3]
            cam_idxs.append(cam_idx) # [nrays, 2] -> 2 = t + cam
            depths.append(depth) # [nrays]
            sems.append(semantics) # [nrays]
            coors.append(coor) # [nrays, 2]

        results['origins'] = torch.stack(all_origins).float()
        results['directions'] =  torch.cat(all_directions).float()
        results['ray_dataset'] = [torch.cat(cam_idxs), torch.cat(depths).float(), torch.cat(sems)]
        results['coor'] = torch.cat(coors).floor().long()
